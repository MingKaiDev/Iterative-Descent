...
public class NumberLockUI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Shift selection
        if (kb.aKey.wasPressedThisFrame) ShiftSelection(-1);
        if (kb.dKey.wasPressedThisFrame) ShiftSelection(1);

        // Spin digit
        if (kb.wKey.wasPressedThisFrame) SpinDigit(1);
        if (kb.sKey.wasPressedThisFrame) SpinDigit(-1);
    ...
    {
        isOpen = false;
        // Panel deactivation handled by PadlockProp
    }
