...
- Dijkstra tutorial caption + diagram — diagram half DONE: `dijkstra` diagram built directly as Unity UI YAML (GameObject/RectTransform/Image/TMP objects, no editor scripting) and spliced into `DiagramRoot` in `Assets/Prefab/UI/Concept Tutorial Panel.prefab` as a 19-child sibling of `arrays_and_lists` etc. Nodes A/B/C/D (A start, D goal), 4 rotated-rect edge lines with cost labels (A-B=2, A-C=1, B-D=6, C-D=3), running distance tags (d=0/2/1/4), and a step-by-step calculation trace panel (visit order A->C->B->D, shortest path A-C-D cost=4) on the right. Still pending: caption text entry in `ConceptTutorials.cs`'s `_captionText` dictionary (`ConceptTutorials.ShowIfUnseenThenContinue("dijkstra", ...)` still no-ops without it — draft text already proposed in `Story19_ChemistryLabFollowUps.md`), and no real-Editor verification yet (no Unity/dotnet compiler available from this side; prefab was validated by regenerating from `gen.py` and checking fileID uniqueness + full child-reference resolution, not by opening it in-Editor).
